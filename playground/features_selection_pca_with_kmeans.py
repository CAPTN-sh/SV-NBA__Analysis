...
def optimise_pca_hyperparameters(df: pd.DataFrame, n_components_range: Tuple[int, int]) -> int:
    ...
    kmeans = KMeans(n_clusters=min(15, n_samples//2), random_state=42)
...
def main():
    ...
    n_components_range = (4, df.shape[1])  #min(len(df.columns), 10))
    ...
    if SAVE_SELECT_FEATURES:
        ...
        fs_df.to_csv(selected_features_dir / 'selected_features_pca_with_kmeans.csv', index=False)
        print("Selected Features saved to:", selected_features_dir / 'selected_features_pca_with_kmeans.csv')
